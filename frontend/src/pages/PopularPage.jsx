...
const PopularPage = () => {
  ...
  const getImageUrl = (anime) => {
    return anime.Images?.find(img => img.Format === "jpg")?.ImageUrl || 
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold gradient-text mb-2">🔥 Popular Anime</h1>
          <p className="text-gray-400">Most popular anime series and movies</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-5 gap-6">
          {animeList.filter(anime => !anime.placeholder).map((anime, index) => (
            <Link
